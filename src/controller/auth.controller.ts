...
export class AuthController {
  ...
  constructor(
    ...
    private ssoStrategy: SamlStrategy,
  ...
  saml(@Res() res) {
    ...
    const a = this.ssoStrategy.generateServiceProviderMetadata(
      readFileSync(this.configService.get<string>('SSO_SP_CERT')).toString(),
      readFileSync(this.configService.get<string>('SSO_SP_CERT')).toString(),
    );
    res.send(a);
  }
  ...
  lbs_login(@Req() req: Request) {
    ...
    if (
      ...
      !req.headers['client-authorization'] ||
      req.headers['client-authorization'] !==
        this.configService.get<string>('LBS_CLIENT_AUTH')
    ) {
    ...
    if (username.startsWith(this.configService.get<string>('SSO_PREFIX'))) {
      try {
        ...
        if (
          payload.barcode ===
          username.replace(this.configService.get<string>('SSO_PREFIX'), '')
        ) {
          console.log(
            'SAML login successful for patron with barcode: ' + payload.barcode,
          );
          return {
          ...
          console.log(
            'Token is valid but barcode does not match username for user ' +
              username,
          );
          throw new UnauthorizedException({
  ...
  async signonCallback(
    ...
    if (this.regex && !this.regex.test(payload.barcode)) {
      throw new InternalServerErrorException(
        'Invalid barcode format from SAML response: ' + payload.barcode,
      );
    }
